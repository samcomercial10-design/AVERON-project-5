...
(async function(){
  ...
  const set=(id,val)=>{const el=document.getElementById(id);if(el&&val!==undefined&&val!==null&&String(val)!=='')el.textContent=String(val)};set('p-name',p.name);set('p-price','£'+p.price.toFixed(2));const previousPrice=document.getElementById('p-previous-price');if(previousPrice){const old=Number(p.previousPrice);const show=Number.isFinite(old)&&old>p.price;if(show){previousPrice.textContent='£'+old.toFixed(2);previousPrice.hidden=false}else{previousPrice.textContent='';previousPrice.hidden=true}}set('p-description',p.description);set('p-details',p.details);set('p-fit',p.fit);set('p-delivery',p.delivery);set('p-returns',p.returns);
